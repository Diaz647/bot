...
# --- Configuration ---
TOKEN = os.getenv("TELEGRAM_BOT_TOKEN") or '8128882254:AAEZ_6OicThy8hlo-k4JShBlsatOyqzRhBY'  # Replace with your token
...
# Webhook settings
WEBHOOK_HOST = os.getenv("RENDER_EXTERNAL_URL") or None
...
SUPER_ADMIN_ID = 7877979174  # SET YOUR TELEGRAM ID HERE (VERY IMPORTANT)
if SUPER_ADMIN_ID == 7877979174:
    logger.error("Please set SUPER_ADMIN_ID to your Telegram ID!")
    exit(1)
...
def load_data():
    ...
    try:
        ...
        # Ensure SUPER_ADMIN is in admins
        ...
        logger.error(f"{DATA_FILE} is corrupted. Recreating. Error: {e}")
        # Backup old file
...
def send_with_markdown_v2_fallback(bot_method, chat_id_or_message, text_content, reply_markup=None):
    """Try sending with MarkdownV2, fallback to plain text on error"""
    ...
    try:
        ...
        if any(err_str in str(e).lower() for err_str in markdown_errors):
            logger.warning(f"MarkdownV2 parse error (chat {chat_id}): {e}. Trying plain text.")
            try:
                ...
                try:
                    ...
                    logger.error(f"Plain text send also failed (chat {chat_id}): {e3}")
                    return False
        else:
            logger.error(f"Other API Error (chat {chat_id}): {e}")
            raise
    except Exception as ex:
        logger.error(f"Unexpected error sending message (chat {chat_id}): {ex}")
        return False
...
def start_command(message):
    ...
    logger.info(f"User {user_id} ({user_name_raw}) used /start command.")
    ...
    # Send channel buttons if channels exist
...
def check_subscription_callback(call):
    ...
    if not channels:
        try:
            bot.edit_message_text(
                "📢 Şu anda kontrol edilecek zorunlu kanal bulunmamaktadır.",
    ...
    for channel_link in channels:
        ...
        try:
            ...
            logger.error(f"Subscription check error ({effective_channel_id}), user {user_id}: {e}")
            if SUPER_ADMIN_ID != 0:
                try:
                    bot.send_message(
                        SUPER_ADMIN_ID,
                        f"⚠️ Abonelik Kontrol Hatası: Kanal: {effective_channel_id}, Kullanıcı: {user_id}. Hata: {str(e)[:200]}"
                    )
                except Exception as ex_admin:
                    logger.error(f"Could not alert SUPER_ADMIN: {ex_admin}")
            all_subscribed = False
            failed_channels_list.append(channel_link)
    ...
    if all_subscribed:
        try:
            ...
            logger.warning(f"Failed to edit success message ({e}), sending new message.")
            send_with_markdown_v2_fallback(
                bot.send_message,
                call.message.chat.id,
                success_message_text
        ...
        for ch_link in channels:
            ...
            channel_username = ch_link.lstrip('@')
            markup.add(
                types.InlineKeyboardButton(
                    f"{prefix} Kanal: {escape_markdown_v2(ch_link)}",
                    url=f"https://t.me/{channel_username}"
                )
            )
...
def help_command(message):
    ...
    markup = types.InlineKeyboardMarkup().add(
        types.InlineKeyboardButton(
            ...
            callback_data=CB_CREATE_SUPPORT_REQUEST
        )
    )
...
def create_support_request_callback(call):
    if not is_bot_active_for_user(call.from_user.id):
        bot.answer_callback_query(call.id, "ℹ️ Bot bakımda.", show_alert=True)
        return
    ...
    sent_msg = bot.send_message(
        call.message.chat.id,
        "Lütfen sorununuzu veya mesajınızı detaylıca yazın. Mesajınız adminlere iletilecektir."
    )
    bot.register_next_step_handler(sent_msg, process_user_support_message)

def process_user_support_message(message):
    user_id = message.from_user.id
    user_name_raw = message.from_user.first_name or f"User_{message.from_user.id}"
    ...
    admin_msg_text = (
        ...
        f"*Kullanıcı:* {user_name_esc}\n"
        f"*Telegram ID:* `{user_id}`\n"
        f"*Kullanıcı Adı:* {user_username_esc}\n\n"
        f"*Mesajı:*\n{support_text_esc}\n\n"
        f"Cevaplamak için: `/yanitla {user_id} <mesajınız>`"
    )
    
    data = load_data()
    admin_ids = data.get("admins", [])
    
    if not admin_ids and SUPER_ADMIN_ID != 0:
        admin_ids = [SUPER_ADMIN_ID]
    
    if not admin_ids:
        logger.error("No admins to forward support request to!")
        bot.reply_to(message, "Üzgünüz, adminlere ulaşılamadı.")
        return
    ...
    for admin_id_target in admin_ids:
        try:
            ...
            logger.error(f"Could not send support message to admin {admin_id_target}: {e}")
    
    if sent_count > 0:
        bot.reply_to(message, "Mesajınız adminlere iletildi.")
    else:
        bot.reply_to(message, "Mesajınız adminlere iletilirken sorun oluştu.")
...
def reply_to_user_command(message):
    ...
    if len(parts) < 3:
        bot.reply_to(
            message, 
            "Kullanım: `/yanitla <kullanıcı_id> <mesajınız>`", 
            parse_mode="MarkdownV2"
        )
        return
    ...
    if send_with_markdown_v2_fallback(
        bot.send_message,
        user_id_to_reply,
        final_reply_to_user
    ):
        bot.reply_to(
            message,
            f"✅ `{user_id_to_reply}` ID'li kullanıcıya yanıtınız gönderildi.",
            parse_mode="MarkdownV2"
        )
    else:
        bot.reply_to(
            message,
            f"⚠️ `{user_id_to_reply}` ID'li kullanıcıya yanıt gönderilemedi veya Markdown sorunu oldu (logları kontrol edin).",
            parse_mode="MarkdownV2"
        )

# --- Configurable Text Message Handlers ---
...
def _process_set_text_message(message, original_message_id, text_key_to_set):
    ...
    if not new_text.strip():
        bot.send_message(message.chat.id, "❌ Mesaj boş olamaz. İşlem iptal edildi.")
    else:
        data = load_data()
        data[text_key_to_set] = new_text
        save_data(data)
...
def admin_set_start_text_callback(call):
    prompt = (
        "🆕 Lütfen kullanıcılar /start yazdığında gösterilecek yeni metin mesajını girin.\n"
...
def admin_set_channel_announce_text_callback(call):
    prompt = (
        "📢 Lütfen kanallara gönderilecek genel duyuru için yeni metin mesajını girin.\n"
...
def admin_alert_users_text_prompt_callback(call):
    ...
    bot.register_next_step_handler(
        sent_msg,
        process_alert_users_message_text_only,
        call.message.message_id
    )

def process_alert_users_message_text_only(message, original_message_id):
    if not is_admin_check(message.from_user.id):
        return
    
    alert_text_content = message.text
    data = load_data()
    ...
    if not users:
        bot.send_message(message.chat.id, "ℹ️ Mesaj gönderilecek kullanıcı yok.")
    else:
        ...
        for user_id in users:
            try:
                sent_ok = send_with_markdown_v2_fallback(
                    bot.send_message,
                    user_id,
                    alert_text_content
                )
                ...
                logger.error(f"Could not send announcement to user {user_id} (API): {e_outer}")
                if any(err_str in str(e_outer).lower() for err_str in ["bot was blocked", "user is deactivated", "chat not found"]):
                    blocked += 1
                failed += 1
            except Exception as e_gen:
                logger.error(f"General error sending to user {user_id}: {e_gen}")
                failed += 1
        ...
        report = (
            f"✅ Duyuru Tamamlandı:\n"
            f"Başarılı: {success}, Başarısız: {failed}, Engelli/Ulaşılamayan: {blocked}"
        )
...
# --- Channel Announcement (Text Only) ---
...
def admin_public_to_channels_callback(call):
    ...
    try:
        ...
        bot_username = "BOT_KULLANICI_ADI"
    
    final_announce_text = announce_text_template.replace("{bot_username}", bot_username)
    
    markup = types.InlineKeyboardMarkup().add(
        types.InlineKeyboardButton(
            "🤖 BOTA GİT 🤖",
            url=f"https://t.me/{bot_username}?start=channelAnnounce"
        )
    )
    ...
    for channel_item in channels_to_send:
        try:
            ...
            logger.error(f"Could not send announcement to channel {channel_item}: {e_outer}")
            failed += 1
...
def admin_view_admins_callback(call):
    ...
    if not admin_ids:
        text_to_send = "ℹ️ Kayıtlı admin yok."
    else:
        text_to_send = "👑 *Kayıtlı Adminler:*\n"
        details_list = []
        ...
        for admin_id in admin_ids:
            ...
            if admin_id == SUPER_ADMIN_ID:
                parts.append("\\(Süper Admin\\)")
            
            try:
                chat = bot.get_chat(admin_id)
                if chat.first_name:
                    parts.append(f"\\- {escape_markdown_v2(chat.first_name)}")
                if chat.username:
                    parts.append(f"\\(@{escape_markdown_v2(chat.username)}\\)")
            except:
                pass
